    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mRegisterButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                final String username = mUsernameEditText.getText().toString(); // user input string
                final String password = Utils.md5Encryption(mPasswordEditText.getText().toString());    // encrypt the string (MD5)
                final User user = new User(username, password, System.currentTimeMillis()); // create user object
                mDatabase.child("users").addListenerForSingleValueEvent(new ValueEventListener() {  // create table "users"
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {   // current status of table "users"
